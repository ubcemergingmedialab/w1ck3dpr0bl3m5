...
class MemState implements InstanceData, Cloneable, HexModelListener
{
	...
	long maxScroll = Math.max(0, newLast + 1 - (rows_ - 1) * columns);
	...
	if (isValidAddr(addr)) {
	    ...
	    if (addr < curTop) {
		curScroll = addr;
	    } else if (addr >= curTop + rows_ * columns) {
		curScroll = addr - (rows_ - 1) * columns;
	...
	long maxAddr = getLastAddress() - rows_ * columns;
	...
	if (x < boxX || x >= boxX + boxW || y <= ENTRY_YOFFS
		|| y >= ENTRY_YOFFS + rows_ * ENTRY_HEIGHT) {
	    return -1;
	}
    ...
    public void paint(Graphics g, int leftX, int topY, int height)
    {
	//
	// This absolutely needs to come first.
	//
	rows_ = (height - 4 * ENTRY_YOFFS) / ENTRY_HEIGHT;
	...
	if (row < topRow || row >= topRow + rows_) return -1;
